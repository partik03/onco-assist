...
export class MedicalWorkflow {
  ...
  getGoogleService(): GoogleService {
    return this.googleService;
  }

  getTiDBService(): TiDBService {
    return this.tidbService;
  }
...
Generated: ${new Date().toISOString()}
  ...
  async findSimilarCases(symptoms: string[], reportType: string, limit: number = 5): Promise<any[]> {
    try {
      const similarCases = await this.tidbService.findSimilarCases(symptoms, reportType, limit);
