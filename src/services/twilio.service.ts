...
export class TwilioService {
  ...
  async sendWhatsApp(to: string, message: string): Promise<string> {
    if (!this.isConfigured || !this.client) {
      logger.info(`[MOCK] WhatsApp message would be sent to ${to}: ${message}`);
      return `mock_whatsapp_${Date.now()}`;
    }
  ...
  async sendMMS(to: string, message: string, mediaUrl?: string): Promise<string> {
    if (!this.isConfigured || !this.client) {
      logger.info(`[MOCK] MMS would be sent to ${to}: ${message}${mediaUrl ? ` with media: ${mediaUrl}` : ''}`);
      return `mock_mms_${Date.now()}`;
    }
  ...
  async getMessageStatus(messageSid: string): Promise<string> {
    if (!this.isConfigured || !this.client) {
      logger.info(`[MOCK] Message status for ${messageSid}: delivered`);
      return 'delivered';
    }
